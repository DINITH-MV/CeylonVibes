...
const ReceivedSlips = () => {
  ...
  useEffect(() => {
    const fetchSlips = async () => {
      try {
        const response = await axios.get("http://localhost:5017/slips");
  ...
  const acceptSlip = async (id, user_id, value) => {
    ...
    try {
      const fetchBills = async () => {
        try {
          const response = await axios.get("http://localhost:5017/bill");
          const unpaidBills = response.data.filter(
            (bill) => bill.status === 'pending' && bill.User_ID === user_id
          );

          unpaidBills.forEach(async (bill) => {
            await axios.put(`http://localhost:5017/bill/${bill._id}`, { status: "paid" });
          });
        } catch (error) {
          console.error('Error fetching bills:', error);
        }
      };
      await axios.post("http://localhost:5017/pay", {
        User_ID: user_id,
        pmethod: "Slip",
        date: currentDate,
        Value: value,
      });
      await axios.put(`http://localhost:5017/slips/${id}`, { status: "Accepted" });
      setReceivedSlips(prevSlips =>
        prevSlips.filter(slip => slip._id !== id)
      );

      // Create new notification
      await axios.post("http://localhost:5017/noti", {
    ...
    try {
      const fetchBills = async () => {
        try {
          ...
          await axios.put(`http://localhost:5017/slips/${id}`, { status: "Rejected" });
          setReceivedSlips(prevSlips =>
            prevSlips.filter(slip => slip._id !== id)
          );
          await Promise.all(unpaidBills.map(async (bill) => {
            await axios.put(`http://localhost:5017/bill/${bill._id}`, { status: "unpaid" });
          }));
        } catch (error) {
          console.error('Error fetching bills:', error);
        }
